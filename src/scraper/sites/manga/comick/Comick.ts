...
async GetChaptersByHot(type?: string, page?: number, order?: string) {
    try {
        ...
        const resultList: IResultSearch<IMangaList> = {
            results: data.map((e: { id: number; chap: string; hid: string;   lang: string; publish_at: string; created_at: string; last_at: string; md_comics: { title: string; slug: string; country: string; md_covers: { b2key: string }[] } }) => {
                ...
                const country = mdComic ? mdComic.country : '';
    ...
    async GetMangaInfo(manga: string, lang: string): Promise<Manga> {
        ...
        const dataApi = await axios.get(`${this.api}/comic/${mangaInfoParseObj.comic.hid}/chapters${currentLang}&limit=10000`);
            ...
            const MangaInfo: Manga = {
                ...
                country:  mangaInfoParseObj.comic.country,
                year: mangaInfoParseObj.comic.year,
                 follow_rank: mangaInfoParseObj.comic.follow_rank,
                rating_count:  mangaInfoParseObj.comic.rating_count,
                user_follow_count:  mangaInfoParseObj.comic.user_follow_count,
                ...
                authors: mangaInfoParseObj.authors.map((e: { name: string;  slug: string})=> ({
                name: e.name,
                slug: e.slug,
                })),
        ...
        try {
            ...
            if (JSON.parse($("#__NEXT_DATA__").html()).isFallback == false) {
                ...
                nextId: nextId!, // Here we use the non-null assertion operator
                    ...
                    created_at: mangaChapterInfoParseObj.chapter.created_at,
                    updated_at: mangaChapterInfoParseObj.chapter.updated_at,
                     last_at: mangaChapterInfoParseObj.chapter.last_at,
                     publish_at: mangaChapterInfoParseObj.chapter.publish_at,
                     crawled_at: mangaChapterInfoParseObj.chapter.crawled_at,
                    date: {
                        year: mindate.getFullYear() ? mindate.getFullYear() : null,
                        month: mindate.getMonth() ? mindate.getMonth() : null,
                        day: mindate.getDay() ? mindate.getDay() : null
                ...
                const MangaChapterInfoChapter: MangaChapter = {
                    ...
                    cover: "https://meo.comick.pictures/" + dataBuild.data.pageProps.chapter.md_comics.md_covers[0].b2key,
                       created_at: dataBuild.data.pageProps.chapter.created_at,
                    updated_at: dataBuild.data.pageProps.chapter.updated_at,
                     last_at: dataBuild.data.pageProps.chapter.last_at,
                     publish_at: dataBuild.data.pageProps.chapter.publish_at,
                     crawled_at: dataBuild.data.pageProps.chapter.crawled_at,
                    date: {
                        year: mindate.getFullYear() ? mindate.getFullYear() : null,
                        month: mindate.getMonth() ? mindate.getMonth() : null,
                        day: mindate.getDay() ? mindate.getDay() : null
